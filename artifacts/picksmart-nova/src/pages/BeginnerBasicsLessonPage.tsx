...
import { beginnerBasicsLessonPage as lesson } from "@/data/beginnerBasicsLessonPage";
...
import {
  ...
  ChevronRight, Clock, BookOpen, Headphones, Award,
} from "lucide-react";
...
export default function BeginnerBasicsLessonPage() {
  ...
  // Record score once when submitted
  useEffect(() => {
    if (submitted && !scoreRecorded) {
      setScoreRecorded(true);
      completeLesson("mod-1", score, lesson.quiz.questions.length);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <div className="sticky top-0 z-20 border-b border-slate-800 bg-slate-950/95 backdrop-blur px-4 py-4">
        <div className="max-w-3xl mx-auto flex items-center gap-4">
          ...
          <span className="px-2 py-1 rounded-md text-xs font-black uppercase bg-yellow-400 text-slate-950 shrink-0">
            FREE
          </span>
        </div>
          ...
          <>
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900 p-8">
              <div className="flex flex-wrap items-center gap-2 mb-4">
                <span className="px-3 py-1 rounded-full bg-green-500/10 border border-green-500/30 text-xs font-bold text-green-400">
                  {lesson.level}
                </span>
                <span className="px-3 py-1 rounded-full bg-yellow-400/10 border border-yellow-400/30 text-xs font-bold text-yellow-300">
                  {lesson.badge}
                </span>
      ...
      <div className="max-w-3xl mx-auto px-4 pb-14 pt-6">
        <LessonVideoPlayer moduleId="mod-1" />
      </div>
    </div>
  );
}
