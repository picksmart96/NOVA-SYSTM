...
import { pickPathOptimizationLessonPage as lesson } from "@/data/pickPathOptimizationLessonPage";
...
import {
  ...
  ChevronRight, Clock, BookOpen, Headphones, Award, Route,
} from "lucide-react";
...
export default function PickPathOptimizationLessonPage() {
  ...
  useEffect(() => {
    if (submitted && !scoreRecorded) {
      ...
      completeLesson("mod-4", score, lesson.quiz.questions.length);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900 p-8">
              <div className="flex flex-wrap items-center gap-2 mb-4">
                <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-yellow-400/10 border border-yellow-400/30 text-xs font-bold text-yellow-300">
                  <Route className="h-3 w-3" /> {lesson.level}
                </span>
      ...
      <div className="max-w-3xl mx-auto px-4 pb-14 pt-6">
        <LessonVideoPlayer moduleId="mod-4" />
      </div>
    </div>
  );
}
